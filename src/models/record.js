...
class Record {
  ...
  insert(){// save => insert > promise user.save().then(f).catch(f)
    let sqlValues = [this.constructor.tableName()];
    ...
    sqlValues.push(attributes);
    return new Promise((resolve,reject) => { // =>  ... thisの引き継ぎ 
      db.query("INSERT INTO ?? (??) VALUE (?);",sqlValues).then((result) => {
        let info = result[0];
        let fields = result[1];
